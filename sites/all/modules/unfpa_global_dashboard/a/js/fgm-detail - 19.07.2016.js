...
(function ($) {
  ...
  Drupal.behaviors.unfpa_global_fgm_data = {
    attach: function (context, settings) {
      ...
      mainObjinequalities = Drupal.settings.data['mainObj'];
    }
  };
...
//      }
    ...
    $('.dashboard-country-outer-box .iCheck-helper').click(function () {
      ...
      if ($(this).parent().parent().parent().parent().prop('id') == "filter_inequalities")
      {
        inequalities_filter = $(this).siblings().val();
        $('.inequalities_graph').css('display', 'none');
        $('#graphCO2 .' + inequalities_filter).css('display', 'block');
      }
      if ($(this).parent().parent().parent().parent().prop('id') == "filter_opposition")
      {
        debugger;
        opposition_filter = $(this).siblings().val();
        $('.opposition_graph').css('display', 'none');
...
// on Chart Full Screen Highchart Zoom click event
    $(document).on('click', '.icon_two', function () {
      ...
      if (visDivID !== 'mapDivId') {
        var containerParentDivID = $(this).parent().parent().parent().parent().attr('id');
        // get chart container div id
        var visDivID = $(this).parent().parent().parent().parent().attr('id').replace('graph', '');
      }
  ...
  function load_all_charts(area_id) {
    ...
    $.each(mainObj, function (key, val) {
      ...
      if (val.data_type == 'pie') {
        ...
        for (i = 0; i < 12; i++) {
          if (i != 6 && i != 7 && i != 8) {
            delete inequaltites_education_chart.series[0].data[i];
            delete inequaltites_education_chart.series[1].data[i];
            delete inequaltites_education_chart.categories[i];
          }
        }

        for (i = 0, j = 6; i < 3; i++, j++) {
          inequaltites_education_chart.series[0].data[i] = inequaltites_education_chart.series[0].data[j];
          inequaltites_education_chart.series[1].data[i] = inequaltites_education_chart.series[1].data[j];
          inequaltites_education_chart.categories[i] = inequaltites_education_chart.categories[j];
          delete inequaltites_education_chart.series[0].data[j];
          delete inequaltites_education_chart.series[1].data[j];
          delete inequaltites_education_chart.categories[j];
        }
        ...
        for (i = 0; i < 10; i++) {
          delete inequaltites_residence_chart.series[0].data[i];
          delete inequaltites_residence_chart.series[1].data[i];
          delete inequaltites_residence_chart.categories[i];
        }

        for (i = 0, j = 10; i < 2; i++, j++) {
          inequaltites_residence_chart.series[0].data[i] = val.series[0].data[j];
          inequaltites_residence_chart.series[1].data[i] = val.series[1].data[j];
          inequaltites_residence_chart.categories[i] = val.categories[j];
          delete inequaltites_residence_chart.series[0].data[j];
          delete inequaltites_residence_chart.series[1].data[j];
          delete inequaltites_residence_chart.categories[j];
        }
        ...
        for (i = 5; i < 12; i++) {
          ...
          delete inequaltites_wealth_chart.series[0].data[i];
          delete inequaltites_wealth_chart.series[1].data[i];
          delete inequaltites_wealth_chart.categories[i];
        }
        ...
        if (key == 'CO5') {
          ...
          // Opposition country  bar graph
          var opposition_country_chart = jQuery.extend(true, {}, val);
          for (i = 1; i < 14; i++) {
            // do something with `substr[i]`
            delete opposition_country_chart.series[0].data[i];
            delete opposition_country_chart.series[1].data[i];
            delete opposition_country_chart.categories[i];
          }
          loadGroupedChart(opposition_country_chart, 'CO5', area_id, filter);

          // Opposition residence bar graph
          var opposition_residence_chart = jQuery.extend(true, {}, val);
          for (i = 0; i < 14; i++) {
            if (i != 2 && i != 3) {
              delete opposition_residence_chart.series[0].data[i];
              delete opposition_residence_chart.series[1].data[i];
              delete opposition_residence_chart.categories[i];
            }
          }

          for (i = 0, j = 2; i < 2; i++, j++) {
            opposition_residence_chart.series[0].data[i] = opposition_residence_chart.series[0].data[j];
            opposition_residence_chart.series[1].data[i] = opposition_residence_chart.series[1].data[j];
            opposition_residence_chart.categories[i] = opposition_residence_chart.categories[j];
            delete opposition_residence_chart.series[0].data[j];
            delete opposition_residence_chart.series[1].data[j];
            delete opposition_residence_chart.categories[j];
          }
...
// Opposition education bar graph
          ...
          for (i = 0; i < 14; i++) {
            if (i != 5 && i != 6 && i != 7) {
              delete opposition_education_chart.series[0].data[i];
              delete opposition_education_chart.series[1].data[i];
              delete opposition_education_chart.categories[i];
            }
          }

          for (i = 0, j = 5; i < 3; i++, j++) {
            opposition_education_chart.series[0].data[i] = opposition_education_chart.series[0].data[j];
            opposition_education_chart.series[1].data[i] = opposition_education_chart.series[1].data[j];
            opposition_education_chart.categories[i] = opposition_education_chart.categories[j];
            delete opposition_education_chart.series[0].data[j];
            delete opposition_education_chart.series[1].data[j];
            delete opposition_education_chart.categories[j];
          }
          ...
          for (i = 0; i < 9; i++) {
            delete opposition_wealth_chart.series[0].data[i];
            delete opposition_wealth_chart.series[1].data[i];
            delete opposition_wealth_chart.categories[i];
          }

          for (i = 0, j = 9; i < 5; i++, j++) {
            opposition_wealth_chart.series[0].data[i] = opposition_wealth_chart.series[0].data[j];
            opposition_wealth_chart.series[1].data[i] = opposition_wealth_chart.series[1].data[j];
            opposition_wealth_chart.categories[i] = opposition_wealth_chart.categories[j];
            delete opposition_wealth_chart.series[0].data[j];
            delete opposition_wealth_chart.series[1].data[j];
            delete opposition_wealth_chart.categories[j];
          }
...
// for all the line, bar, column and stacked charts ( Object CO1, CO2, CO3, CO4, CO5, CO7, CO9, CO10)
  function loadGroupedChart(data, chartID, area_id, filter) {
    ...
    if (chartID == 'CO21' || chartID == 'CO20' || chartID == 'CO2' || chartID == 'CO5' || chartID == 'CO16') {
    ...
    if (chartID == 'CO21' || chartID == 'CO20' || chartID == 'CO16' || chartID == 'CO3' || chartID == 'CO4' || chartID == 'CO5_1' || chartID == 'CO10_1') {
    ...
    var chart = new Highcharts.Chart({
      ...
      if (chartID == 'CO21' || chartID == 'CO20' || chartID == 'CO2' || chartID == 'CO16')
      {
        var width = $(window).width();
        if (width > 930) {
          xVal = 362;
