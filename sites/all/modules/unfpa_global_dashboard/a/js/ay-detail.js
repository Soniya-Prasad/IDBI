...
//    $('.graph_table').click(function () {
//      $(this).parents('.dashboard-country-outer-box').find('.action-link-list .download').css('display', 'none');
//      $(this).siblings().removeClass('active');
//      $(this).addClass('active');
//      class_bar_chart_table = '.' + $(this).prop('id');
//      var visDivID = containerParentDivID = null;
//      window[visDivID] = null;
//      visDivID = $(class_bar_chart_table).prop('id');
//      containerParentDivID = $(class_bar_chart_table).parent("div").prop('id');
//      var objType = 'chart';
//      // get chart object
//      var obj = jQuery.extend(true, {}, window[visDivID]);
//      objTblSettings = {title: {enabled: false}};
      ...
      if (($(this).parents('.dashboard-country-outer-box').prop('id')) != 'population-distribution-section') {
        ...
        if ($(this).parents('.square-input-element').hasClass('graphfilter')) {
          ...
          if (mapLoadedFlg)	//if map has been loaded properly
          {
            ...
            selOpt = "by_part_resd_household_adolescnt_girl_10_14";
            map_title = selOpt;
...
//    });
    ...
    $('.dfapi-viz-toolbar .dfapi-viz-toolbar-medium').on('click', function () {
      ...
      if ($(this).hasClass('print'))
      {
        ...
        source_text = "<div class='source_content'>Source: Demographic and Health Surveys 2013</div>";
        print_title = "<h2>Adolescents and Youth Dashboard - " + country_name + "</h2>";
        //print_title = "<div class='logo-unfpa'>";
        ...
        if ($(this).parents('.dashboard-country-outer-box').find('button.graph').hasClass('active')) {
          ...
          $(this).parents('.dashboard-country-outer-box').find(graph_divid).after(source_text);
          ...
          $(this).parents('.dashboard-country-outer-box').find('.dfapi-visTbl-container').after(source_text);
          ...
          $(this).parents('.dashboard-country-outer-box').find('.map_part').after(source_text);
...
// get chart container div id
        ...
        if (fileType == 'csv') {
          ...
          //  $('#' + visDivID).highcharts().exporting.csv.columnHeaderFormatter(chart.series, 'test', 4);

          console.log("map data chart categories");
          console.log(chart);
          var chart = $('#' + visDivID).highcharts();
          $(this).attr('href', 'data:text/csv;charset=utf-8,' + escape(chart.getCSV()));
          $(this).attr('download', country_name + "-map.csv");
                ...
                events: {
                  load: function () {
                    ...
                    this.renderer.image('http://www.unfpa.org/sites/all/modules/sowp/images/logo.svg', -12, 5, 100, 30)
                      .add();
                    this.renderer.text('(c) United Nations Population Fund', 5, 447)
                      .attr({
                        rotation: 0
                      })
                      .css({
                      ...
                      .add();
                    this.renderer.text('Source: Demographic and Health Surveys 2013', 150, 447)
                      .attr({
                        rotation: 0
                      })
                      .css({
...
//      });
    ...
    function makePieChart(graphDiv, selOpt)
    {
      ...
      if (pieChartData !== undefined && pieChartData != '' && pieChartData != null) {
        ...
        console.log("rural")
        console.log(pieChartData.seriesData[0]['data'][1]['name']);
//        if (pieChartData.seriesData[0]['data'][1]['name'] == "Rural") {
//          legendSettings = {
//            enabled: true,
...
//        }
    ...
    function getChartXMLDataNode(contentDv, selOpt, visualType)
    {
      ...
      console.log("selOptData");
      console.log(contentDv);
      //selOptData['seriesData'][0]['data'][2] = 'yes';
      var changeleg = [];
      //changeled = selOptData['seriesData'][0]['data'][1];
      //selOptData['seriesData'][0]['data'][1] = {changeled, 'itemMarginBottom': '10px'};
      //selOptData['seriesData'][0]['data'][1] = {'color': "#002060", 'itemMarginBottom': "10px", 'name': "Rural", 'y': 82.3};
      console.log(selOptData);
      return selOptData;
    }
                  ...
                  events: {
                    load: function () {
                      ...
                      this.renderer.image('http://www.unfpa.org/sites/all/modules/sowp/images/logo.svg', -12, 5, 100, 30)
                        ...
                        .add();
                      this.renderer.text('(c) United Nations Population Fund', 10, 302)
                        .attr({
                          rotation: 0
                        })
                        .css({
...
//              .add();
//
//            chart.renderer.image('http://highcharts.com/demo/gfx/sun.png', 0, 500, 600, 600)
//              .add();
//
...
//    $('#map_area_indicator_data').highcharts({
//      chart: {
//        events: {
//          load: function () {
//            $(".highcharts-legend").appendTo("#legend");
//            $("#legend").removeAttr('transform');
