...
var IndLowerIsGood = // array of indicators lies in the category where loe is good
        {
...
var PIE_CHART_DIV_1 = 'graph_ppl_dstrbtn_resd_regn'; //get pie chart div
var MAP_DIV = 'map_area_indicator_data';
var DEFULT_MAP_INDCTR = 'perc_girl_age_20_24_mrd_bfr_18'; //set deafut map indicator
var mapLoadedFlg = false; //first time map loader flag
var IconImgFlag = false; //full screen flag

var MAP_TYPE = 'map'; //get graph type
var GRAPH_TYPE = 'graph'; //get graph type
var TBL_TYPE = 'table'; //get graph type
...
//--------------------indicator end
    ...
    $('#slctindi_id').ddslick({
      width: 350,
      height: 350,
      defaultSelectedIndex: DEFULT_MAP_INDCTR,
      onSelected: function (sData) {
        ...
        if (mapLoadedFlg)	//if map has been loaded properly
        {
          ...
          //changeD3Data();
          //console.log($('#visTblDvId').find($('div.dfapi-visTbl-container')).length);
        }
...
$('.graph').click(function () {
    ...
    $('#map_render').css('display','none');
    $('.'+$(this).prop('id')).css('display','block');
});
...
else
{
    ...
    $('.dashboard-country-outer-box label').click(function () {
        $('#map_render').css('display','block');
        $('.bar_chart').css('display','none');
        id_for_chart_button='graph_'+$(this).siblings().children('input').prop('className');
        chart_button_parent='.'+$(this).parent().parent().parent().parent().prop('id');
        console.log(345);
      ...
      filter_id = '#' + ($(this).siblings().children('input').prop("className"));
...
//redrawing alll the charts when window is resized
    ...
    loadHCMapData = function (mapGeojson, mapData) {
      ...
      window['map_area_indicator_data'] = new Highcharts.Map({
        chart: {
          renderTo: 'map_area_indicator_data',
          borderWidth: 0,
          backgroundColor: '#fff',
          style: {
...
//function to get selected indicator map data
    ...
    if (chckMapDataFlg) {									//if map indicator data found
      //Map Dynamic Legend
      var mapIndctData = $('#slctindi_id').data('ddslick');
      //selOpt = "perc_girl_age_20_24_mrd_bfr_18";
      selOpt = mapIndctData.selectedData.value;
          ...
          if ($.trim(data.map) != '') {
            ...
            if (filexistAreaIdStatus == false) {
              ...
              if (dataformatStatus == true) {
                ...
                ChangeLegend(selOpt);
                loadHCMapData(mapGeojson, mpJsonObjtoArr); //display selected ius data using api
//                $.ajax({
//                  type: "POST",
...
//                });
              ...
              var mapData = getMapData();
              //console.log(mapData);
              loadHCMapData(mapGeojson, mapData); //display selected ius data using api
