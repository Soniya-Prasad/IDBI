...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("a.dfapi-icn-cls").on("click", function (event) {
      $('#dfapi-fs-outer-container').remove();
    });
    $('.map').click(function () {
      $(this).parent().siblings().find('.square-input-element').css('display', 'none');
      $(this).siblings().removeClass('active');
...
//        console.log(chart_button_parent);
    ...
    $('.dfapi-viz-toolbar .dfapi-viz-toolbar-medium').on('click', function () {
      if ($(this).hasClass('print'))
      {
        if ($(this).parents('.dashboard-country-outer-box').find('button.graph').hasClass('active')) {
          ...
          var objType = 'chart';
          $(this).parents('.dashboard-country-outer-box').find(graph_divid).parent().find('.print-title').css('display', 'block');
          var containerParentDivID = $(graph_divid).parent().prop('id');
        } else if ($(this).parents('.dashboard-country-outer-box').find('button.table').hasClass('active')) {
          $('table').css('background-color', '#C16B06');
          $('table, table th, table td').css('border', '1px solid #000000');
          $(this).parents('.dashboard-country-outer-box').find('.dfapi-visTbl-container').parent().find('.print-title').css('display', 'block');
          var containerParentDivID = $(this).parents('.dashboard-country-outer-box').find('.dfapi-visTbl-container').parent().prop('id');
          var objType = 'chart';
        } else {
          $(this).parents('.dashboard-country-outer-box').find('.map_part').parent().find('.print-title').css('display', 'block');
...
//      return false;
    ...
    //Full Screen
    $('a.icon_four, .fullscreen').on('click', function () {
      ...
      if ($(this).parents('.dashboard-country-outer-box').find('button.map').hasClass('active')) {
        ...
        var visDivID = $(this).parents('.dashboard-country-outer-box').find('.map_part').prop('id');
        //$('.blockUI.blockMsg.blockPage').find('.dfapi-visTbl-container').css('display', 'none');
      }
...
//      var containerParentDivID = 'p-VO_8';
//      visDivID = 'graph_perc_age_diff_btwn_female_and_partnr';
      //console.log(containerParentDivID+'  '+visDivID);
      var chartTitle = '';
    ...
    loadHCMapData = function (mapGeojson, mapData, div_var) {
      ...
      //$('.highcharts-legend').appendTo($('.country-chart-section-left #legend'));
    };
...
//      var allIusDataApi = [];
//      if (AREA_JSON_DATA_OBJ['by_' + MAP_DIV][selOpt]['IUSGID'] != '') {
